...
# just below commands are for use when running with changes to gnucash sources.
# when using these commands, ensure that `gnucash` is not listed in
# .dockerignore.
...
RUN mkdir -p /home/root/src/build
...
COPY --from=buildenv /home/root/src/build/a-gnucash-integration /usr/local/bin/
